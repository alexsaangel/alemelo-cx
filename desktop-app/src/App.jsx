...
export default function App() {
  ...
  // Carregar do Electron store se disponível
  const [loadedFromElectron, setLoadedFromElectron] = useState(false);

  const [aba, setAba] = useState("base");
  const [data, setData] = useState(load);

  // Salvar dados — Electron disk ou localStorage web
  useEffect(() => {
    try { localStorage.setItem("csmv_v1", JSON.stringify(data)); } catch(e) {}
    if (window.csmvAPI) {
      window.csmvAPI.setData("csmv_v1", data).catch(() => {});
    }
  }, [data]);

  // Carregar do Electron store na inicialização
  useEffect(() => {
    if (window.csmvAPI && !loadedFromElectron) {
      window.csmvAPI.getData("csmv_v1").then(saved => {
        if (saved && saved.clientes) {
          setData(saved);
        }
        setLoadedFromElectron(true);
      }).catch(() => setLoadedFromElectron(true));
      
      // Listeners do menu Electron
      window.csmvAPI.onMenuExport(() => {
        window.csmvAPI.exportData(data);
      });
      window.csmvAPI.onMenuImport(() => {
        window.csmvAPI.importData().then(result => {
...
// ── PATCH: Storage adaptador Electron/Web ────────────────
// Este patch substitui o localStorage pelo electron-store quando rodando como app desktop
EOF
